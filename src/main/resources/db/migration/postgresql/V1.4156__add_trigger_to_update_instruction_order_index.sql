...
BEGIN
	IF TG_OP = 'UPDATE' AND NEW.order_index <> OLD.order_index THEN
		...
		FROM ( SELECT id as ins_id, ROW_NUMBER() OVER (ORDER BY order_index asc , updated_at asc, id asc ) as seqnum 
...
------------------------------------------------------------------------------
CREATE TRIGGER "instruction_after_update_order_index"
  AFTER INSERT OR UPDATE ON instruction
  FOR EACH ROW
  WHEN (pg_trigger_depth() = 0)
EXECUTE PROCEDURE instruction_update_order_index();
------------------------------------------------------------------------------