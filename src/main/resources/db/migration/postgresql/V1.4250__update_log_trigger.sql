...
BEGIN
    ...
    IF TG_OP = 'UPDATE' THEN
      ...
      IF NEW.is_finished IS TRUE THEN        
        ...
        NEW.log = CASE 
        	        WHEN NEW.log IS NULL THEN 
        	          '[7]' || trim(both '"' from to_json(now())::text) || 'this instruction result is finished.'
        	        ELSE 
...
BEGIN
    ...
    IF TG_OP = 'UPDATE' THEN
      ...
      IF NEW.is_finished IS TRUE THEN        
        ...
        NEW.log = CASE 
        	        WHEN NEW.log IS NULL THEN 
        	          '[9]' || trim(both '"' from to_json(now())::text) || 'this run is finished.'
        	        ELSE 
...
BEGIN
    ...
    IF TG_OP = 'UPDATE' THEN
      ...
      IF NEW.is_finished IS TRUE THEN        
        ...
        NEW.log = CASE 
        	        WHEN NEW.log IS NULL THEN 
        	          '[9]' || trim(both '"' from to_json(now())::text) || 'this run set result is finished.'
        	        ELSE 
