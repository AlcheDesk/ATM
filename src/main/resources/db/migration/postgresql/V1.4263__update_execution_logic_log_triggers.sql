...
BEGIN
    ...
    ELSE
        UPDATE prod_instruction_result SET 
        log = 'Received new corresponding step log :' || NEW.id  || ' type : ' || (SELECT name from step_log_type WHERE id = NEW.step_log_type_id)
        WHERE id = NEW.instruction_result_id;
    END IF;
	------------------------------------  
	RETURN NEW;
...
BEGIN
    ...
    ELSE
        UPDATE run SET 
        log = 'Received instruction result update signal from instruction result :' || NEW.id  || 
              ' new status : ' || (SELECT name from status WHERE id = NEW.step_log_type_id) || 
              ' old status : ' || (SELECT name from status WHERE id = OLD.step_log_type_id) ||
...
BEGIN
    ...
    IF NEW.run_set_result_id IS NOT NULL THEN
      ...
      ELSE
        UPDATE run_set_result SET 
        log = 'Received run update signal from run :' || NEW.id  || 
              ' new status : ' || (SELECT name from status WHERE id = NEW.step_log_type_id) || 
              ' old status : ' || (SELECT name from status WHERE id = OLD.step_log_type_id) ||
              ' finished : ' || NEW.is_finished
