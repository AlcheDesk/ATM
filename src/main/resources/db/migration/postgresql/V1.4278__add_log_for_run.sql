    ...
    IF TG_OP = 'UPDATE' THEN
      ...
      IF NEW.is_finished IS TRUE THEN        
        IF NEW.status_id = 2 AND OLD.status_id = 2 THEN
           ...
           NEW.log = CASE 
        	           WHEN NEW.log IS NULL THEN 
        	             trim(both '"' from to_json(now())::text) || ' [PASS] this run to PASS from WIP by finished signal.'
        	           ELSE 
        ...
        ELSIF NEW.status_id = 3 THEN
          NEW.log = CASE 
        	          WHEN NEW.log IS NULL THEN 
        	            trim(both '"' from to_json(now())::text) || ' [PASS] this run to PASS.'
        	          ELSE 
        	            NEW.log || chr(10) || trim(both '"' from to_json(now())::text) || ' [PASS] this run to PASS.' 
        	        END;
        ELSIF NEW.status_id = 8 THEN
          NEW.log = CASE 
        	          WHEN NEW.log IS NULL THEN 
        	            trim(both '"' from to_json(now())::text) || ' [TIMEOUT] this run to TIMEOUT by the system and force to finished. please contact admin.'
        	          ELSE 
        	            NEW.log || chr(10) || trim(both '"' from to_json(now())::text) || ' [TIMEOUT] this run to TIMEOUT by the system and force to finished.' 
        	        END;
        END IF;
    ...
    IF TG_OP = 'UPDATE' THEN
      ...
      IF NEW.is_finished IS TRUE THEN        
        ...
        ELSIF NEW.status_id = 8 THEN
          NEW.log = CASE 
        	          WHEN NEW.log IS NULL THEN 
        	            trim(both '"' from to_json(now())::text) || ' [TIMEOUT] this run set to TIMEOUT by the system and force to finished. please contact admin.'
        	          ELSE 
        	            NEW.log || chr(10) || trim(both '"' from to_json(now())::text) || ' [TIMEOUT] this run set to TIMEOUT by the system and force to finished. please contact admin.' 
        	        END;
        END IF;
