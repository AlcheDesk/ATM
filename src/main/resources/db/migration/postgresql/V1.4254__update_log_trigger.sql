...
BEGIN
    ...
    IF TG_OP = 'UPDATE' THEN
      ...
      END IF;
      ----------------
      IF OLD.log IS NOT NULL AND NEW.log IS NOT NULL AND NEW.log IS DISTINCT FROM OLD.log THEN
        NEW.log = '[00]' || OLD.log || chr(10) || trim(both '"' from to_json(now())::text) || ' ' || trim(leading OLD.log FROM NEW.log);
      ...
      IF NEW.is_finished IS TRUE THEN        
        ...
        ELSIF NEW.status_id = 3 THEN
          NEW.log = CASE 
        	        ...
        	        END;
        END IF;
        NEW.log = '[7]' || chr(10) || trim(both '"' from to_json(now())::text) || 'this instruction result is finished.';
      END IF;
      ----------------
    END IF;
    --------------------------------------------------------------------------------------------------------
      ...
      IF NEW.is_finished IS TRUE THEN        
        ...
        ELSIF NEW.status_id = 3 THEN
          NEW.log = CASE 
        	        ...
        	        END;
        END IF;
        NEW.log = '[9]' || chr(10) || trim(both '"' from to_json(now())::text) || 'this run is finished.';
      END IF;
      ----------------
    END IF;
    --------------------------------------------------------------------------------------------------------
      ...
      IF NEW.is_finished IS TRUE THEN        
        ...
        ELSIF NEW.status_id = 3 THEN
          NEW.log = CASE 
        	        ...
        	        END;
        END IF;
        NEW.log = '[9]' || chr(10) || trim(both '"' from to_json(now())::text) || 'this run set result is finished.';
      END IF;
      ----------------
    END IF;
    --------------------------------------------------------------------------------------------------------
