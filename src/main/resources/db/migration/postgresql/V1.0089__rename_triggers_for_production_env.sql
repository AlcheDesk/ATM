ALTER FUNCTION "instruction_result_update_run_status" ( ) RENAME TO prod_instruction_result_update_run_status;
ALTER FUNCTION "step_log_update_instruction_result_status" ( ) RENAME TO prod_step_log_update_prod_instruction_result_status;
...
--	RAISE NOTICE 'trigger by instruction result, new instruction result status is %', prod_instruction_result_new_status;
...
--    select id into error_log_level_id from log_level where log_level.name = 'ERROR';
    ...
    select type_id into new_prod_step_log_type_id from prod_step_log where prod_step_log.id = NEW.id;
...
--    RAISE NOTICE 'trigger by step log, target instrcution result id is %', NEW.prod_instruction_result_id;
	select status_id into target_prod_instruction_result_current_status_id from prod_instruction_result where prod_instruction_result.id = NEW.prod_instruction_result_id;
...
--    RAISE NOTICE 'trigger by step log, target instrcution result current status is %', target_prod_instruction_result_status_name; 
    ...
    IF target_prod_instruction_result_current_status_id = new_status_id and new_prod_step_log_type_id <> 4 and new_prod_step_log_type_id <> 8 THEN -- 
		update prod_instruction_result  set status_id = wip_status_id where id = NEW.prod_instruction_result_id;
--		RAISE NOTICE 'trigger by step log, update instruction result status to WIP'; 
	...
	ELSIF target_prod_instruction_result_current_status_id = new_status_id and new_prod_step_log_type_id = 8 THEN -- 
		update prod_instruction_result  set status_id = error_status_id where id = NEW.prod_instruction_result_id;
--		RAISE NOTICE 'trigger by step log, update instruction result status to ERROR'; 
	...
	ELSIF  target_prod_instruction_result_current_status_id = new_status_id and new_prod_step_log_type_id = 4 THEN
		update prod_instruction_result set status_id = fail_status_id where id = NEW.prod_instruction_result_id;
