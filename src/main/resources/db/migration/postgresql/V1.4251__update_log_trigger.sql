      ...
      IF NEW.is_finished IS TRUE THEN        
        ...
        ELSIF NEW.status_id = 3 THEN
          NEW.log = CASE 
        	        ...
        	        END;
        END IF;
        NEW.log = '[7]' || trim(both '"' from to_json(now())::text) || 'this instruction result is finished.';
      END IF;
      ----------------
    END IF;
    --------------------------------------------------------------------------------------------------------
      ...
      IF NEW.is_finished IS TRUE THEN        
        ...
        ELSIF NEW.status_id = 3 THEN
          NEW.log = CASE 
        	        ...
        	        END;
        END IF;
        NEW.log = '[9]' || trim(both '"' from to_json(now())::text) || 'this run is finished.';
      END IF;
      ----------------
    END IF;
    --------------------------------------------------------------------------------------------------------
      ...
      IF NEW.is_finished IS TRUE THEN        
        ...
        ELSIF NEW.status_id = 3 THEN
          NEW.log = CASE 
        	        ...
        	        END;
        END IF;
        NEW.log = '[9]' || trim(both '"' from to_json(now())::text) || 'this run set result is finished.';
      END IF;
      ----------------
    END IF;
    --------------------------------------------------------------------------------------------------------
