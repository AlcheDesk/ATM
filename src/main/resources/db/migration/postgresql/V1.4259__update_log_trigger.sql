...
BEGIN
    ...
    IF TG_OP = 'UPDATE' THEN
      ...
      IF OLD.log IS NOT NULL AND NEW.log IS NOT NULL AND NEW.log IS DISTINCT FROM OLD.log AND (NEW.log = '') IS NOT FALSE THEN
        NEW.log = '[00]' || OLD.log || chr(10) || trim(both '"' from to_json(now())::text) || ' ' || NEW.log;
      ELSIF OLD.log IS NULL AND NEW.log IS NOT NULL AND (NEW.log = '') IS NOT FALSE THEN
