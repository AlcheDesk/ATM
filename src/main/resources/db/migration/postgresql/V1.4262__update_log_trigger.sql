...
BEGIN
    ...
    IF TG_OP = 'UPDATE' THEN
      ...
      IF OLD.log IS NOT NULL AND NEW.log IS NOT NULL AND NEW.log IS DISTINCT FROM OLD.log AND NEW.log <> '' AND NEW.log IS NOT NULL THEN
        NEW.log = OLD.log || chr(10) || trim(both '"' from to_json(now())::text) || ' ' || NEW.log;
      ELSIF OLD.log IS NULL AND NEW.log IS NOT NULL AND NEW.log <> '' AND NEW.log IS NOT NULL THEN
        ...
        IF NEW.status_id = 2 AND OLD.status_id = 2 THEN
           ...
           NEW.log = CASE 
        	           WHEN NEW.log IS NULL THEN 
        	             trim(both '"' from to_json(now())::text) || 'this instruction result set to PASS from WIP by finished signal.'
        	           ELSE 
        	             NEW.log || chr(10) || trim(both '"' from to_json(now())::text) || ' this instruction result set to PASS from WIP by finished signal.' 
        	         END;
        ELSIF NEW.status_id = 3 THEN
          NEW.log = CASE 
        	          WHEN NEW.log IS NULL THEN 
        	            trim(both '"' from to_json(now())::text) || 'this instruction result set to PASS.'
        	          ELSE 
        	            NEW.log || chr(10) || trim(both '"' from to_json(now())::text) || ' this instruction result set to PASS.' 
        	        END;
        END IF;
...
BEGIN
    ...
    IF TG_OP = 'UPDATE' THEN
      ...
      IF OLD.start_at IS NULL AND OLD.end_at IS NULL AND OLD.is_finished IS FALSE AND NEW.is_finished IS TRUE THEN -- special case when the run start with failed condition
      	...
      	NEW.log = CASE 
        	        WHEN NEW.log IS NULL THEN 
        	          trim(both '"' from to_json(now())::text) || 'Special condition, the RUN directly entered into end status.'
        	        ELSE 
        	          NEW.log || chr(10) || trim(both '"' from to_json(now())::text) || ' Special condition, the RUN directly entered into end status.' 
        	      END;
      END IF;
      ---------------- [EXECUTION LOGIC] set the finished log and check if we need to set the status to PASS
      IF NEW.is_finished IS TRUE THEN        
        IF NEW.status_id = 2 AND OLD.status_id = 2 THEN
           ...
           NEW.log = CASE 
        	           WHEN NEW.log IS NULL THEN 
        	             trim(both '"' from to_json(now())::text) || 'this run set to PASS from WIP by finished signal.'
        	           ELSE 
        	             NEW.log || chr(10) || trim(both '"' from to_json(now())::text) || ' this run set to PASS from WIP by finished signal.' 
        	         END;
        ELSIF NEW.status_id = 3 THEN
          NEW.log = CASE 
        	          WHEN NEW.log IS NULL THEN 
        	            trim(both '"' from to_json(now())::text) || 'this run set to PASS.'
        	          ELSE 
        	            NEW.log || chr(10) || trim(both '"' from to_json(now())::text) || ' this run set to PASS.' 
        	        END;
        END IF;
...
BEGIN
    ...
    IF TG_OP = 'UPDATE' THEN
      ...
      IF OLD.start_at IS NULL AND OLD.end_at IS NULL AND OLD.is_finished IS FALSE AND NEW.is_finished IS TRUE THEN -- special case when the run set result start with failed condition
      	...
      	NEW.log = CASE 
        	        WHEN NEW.log IS NULL THEN 
        	          trim(both '"' from to_json(now())::text) || 'Special condition, the run set result directly entered into end status.'
        	        ELSE 
        	          NEW.log || chr(10) || trim(both '"' from to_json(now())::text) || ' Special condition, the run set result directly entered into end status.' 
        	      END;
      END IF;
      ---------------- [EXECUTION LOGIC] set the finished log and check if we need to set the status to PASS
      IF NEW.is_finished IS TRUE THEN        
        IF NEW.status_id = 2 AND OLD.status_id = 2 THEN
           ...
           NEW.log = CASE 
        	           WHEN NEW.log IS NULL THEN 
        	             trim(both '"' from to_json(now())::text) || 'this run set result set to PASS from WIP by finished signal.'
        	           ELSE 
        	             NEW.log || chr(10) || trim(both '"' from to_json(now())::text) || ' this run set result set to PASS from WIP by finished signal.' 
        	         END;
        ELSIF NEW.status_id = 3 THEN
          NEW.log = CASE 
        	          WHEN NEW.log IS NULL THEN 
        	            trim(both '"' from to_json(now())::text) || 'this run set result set to PASS.'
        	          ELSE 
        	            NEW.log || chr(10) || trim(both '"' from to_json(now())::text) || ' this run set result set to PASS.' 
        	        END;
        END IF;
