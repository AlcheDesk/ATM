    ...
    IF TG_OP = 'UPDATE' THEN
      ...
      IF NEW.is_finished IS TRUE THEN        
        IF NEW.status_id = 2 AND OLD.status_id = 2 THEN
           ...
           NEW.log = CASE 
        	           WHEN NEW.log IS NULL THEN 
        	             'this instruction result set to PASS from WIP by finished signal.'
        	           ELSE 
        ...
        ELSIF NEW.status_id = 3 THEN
          NEW.log = CASE 
        	          WHEN NEW.log IS NULL THEN 
        	            'this instruction result set to PASS.'
        	          ELSE 
        ...
        NEW.log = CASE 
        	        WHEN NEW.log IS NULL THEN 
        	          'this instruction result is finished.'
        	        ELSE 
        	          NEW.log || chr(10) || trim(both '"' from to_json(now())::text) || ' this instruction result is finished.' 
        	      END;
      END IF;
      ----------------
...
BEGIN
    ...
    IF TG_OP = 'UPDATE' THEN
      ...
      IF OLD.start_at IS NULL AND OLD.end_at IS NULL AND OLD.is_finished IS FALSE AND NEW.is_finished IS TRUE THEN -- special case when the run start with failed condition
      	...
      	NEW.log = CASE 
        	        WHEN NEW.log IS NULL THEN 
        	          'Special condition, the RUN directly entered into end status.'
        	        ELSE 
      ...
      IF NEW.is_finished IS TRUE THEN        
        IF NEW.status_id = 2 AND OLD.status_id = 2 THEN
           ...
           NEW.log = CASE 
        	           WHEN NEW.log IS NULL THEN 
        	             'this run set to PASS from WIP by finished signal.'
        	           ELSE 
        ...
        NEW.log = CASE 
        	        ...
        	        ELSE 
        	          NEW.log || chr(10) || trim(both '"' from to_json(now())::text) || ' this run is finished.' 
        	      END;
      END IF;
      ----------------
      IF OLD.log IS NOT NULL AND NEW.log IS NOT NULL THEN
...
BEGIN
    ...
    IF TG_OP = 'UPDATE' THEN
      ...
      IF OLD.start_at IS NULL AND OLD.end_at IS NULL AND OLD.is_finished IS FALSE AND NEW.is_finished IS TRUE THEN -- special case when the run set result start with failed condition
      	...
      	NEW.log = CASE 
        	        WHEN NEW.log IS NULL THEN 
        	          'Special condition, the run set result directly entered into end status.'
        	        ELSE 
      ...
      IF NEW.is_finished IS TRUE THEN        
        IF NEW.status_id = 2 AND OLD.status_id = 2 THEN
           ...
           NEW.log = CASE 
        	           WHEN NEW.log IS NULL THEN 
        	             'this run set result set to PASS from WIP by finished signal.'
        	           ELSE 
        ...
        ELSIF NEW.status_id = 3 THEN
          NEW.log = CASE 
        	          WHEN NEW.log IS NULL THEN 
        	            'this run set result set to PASS.'
        	          ELSE 
        ...
        NEW.log = CASE 
        	        WHEN NEW.log IS NULL THEN 
        	          'this run set result is finished.'
        	        ELSE 
        	          NEW.log || chr(10) || trim(both '"' from to_json(now())::text) || ' this run set result is finished.' 
        	      END;
      END IF;
      ----------------
      IF OLD.log IS NOT NULL AND NEW.log IS NOT NULL THEN
