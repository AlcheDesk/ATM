...
--	na_status_id bigint;
    ...
    new_step_log_type_id bigint;
	new_step_log_level text;
    
	target_instruction_result_id bigint; --the run linked the the instruction result
	target_instruction_result_current_status_id bigint; -- the run current status id
...
--    select id into error_log_level_id from log_level where log_level.name = 'ERROR';
    ...
    select type_id into new_step_log_type_id from step_log where step_log.id = NEW.id;
    --get the log  leve of the new inserted step log
--    select name into new_step_log_level from log_level where log_level.id = new_step_log_type_id;
...
--    RAISE NOTICE 'trigger by step log, target instrcution result id is %', NEW.instruction_result_id;
	select status_id into target_instruction_result_current_status_id from instruction_result where instruction_result.id = NEW.instruction_result_id;
--    select name into target_instruction_result_status_name from status where status.id = target_instruction_result_current_status_id;
--    RAISE NOTICE 'trigger by step log, target instrcution result current status is %', target_instruction_result_status_name; 
    ...
    IF target_instruction_result_current_status_id = new_status_id and new_step_log_type_id <> 4 and new_step_log_type_id <> 8 THEN -- 
...
--		RAISE NOTICE 'trigger by step log, update instruction result status to WIP'; 
	...
	ELSIF target_instruction_result_current_status_id = new_status_id and new_step_log_type_id = 8 THEN -- 
...
--		RAISE NOTICE 'trigger by step log, update instruction result status to ERROR'; 
	...
	ELSIF  target_instruction_result_current_status_id = new_status_id and new_step_log_type_id = 4 THEN
...
--	ELSIF target_instruction_result_current_status_id = wip_status_id and new_step_log_type_id =  THEN
--		update instruction_result set status_id = wip_status_id where id = NEW.instruction_result_id;
--		RAISE NOTICE 'trigger by step log, update instruction result status to WIP'; 
		
	ELSIF target_instruction_result_current_status_id = wip_status_id and new_step_log_type_id = 4 THEN
		update instruction_result  set status_id = fail_status_id where id = NEW.instruction_result_id;
--		RAISE NOTICE 'trigger by step log, update instruction result status to FAIL'; 
		
	ELSIF target_instruction_result_current_status_id = wip_status_id and new_step_log_type_id = 8 THEN
		update instruction_result  set status_id = error_status_id where id = NEW.instruction_result_id;
--		RAISE NOTICE 'trigger by step log, update instruction result status to ERROR';
		
	ELSIF target_instruction_result_current_status_id = fail_status_id and new_step_log_type_id = 8 THEN
		update instruction_result  set status_id = error_status_id where id = NEW.instruction_result_id;
--		RAISE NOTICE 'trigger by step log, update instruction result status to ERROR'; 
