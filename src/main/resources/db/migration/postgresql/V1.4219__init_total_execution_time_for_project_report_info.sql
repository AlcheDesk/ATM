...
total_execution_time = i.sec
FROM 
(
    select run_project_id as pi, 
    SUM(EXTRACT(EPOCH FROM (run_end_at - run_start_at))) as sec
    FROM run_execution_info where is_finished is true and run_end_at is not null and run_start_at is not null group by run_project_id
) i
