...
public class InstructionOptionTypeHandler extends BaseTypeHandler<String> {

    @Override
    public void setNonNullParameter(PreparedStatement ps, int i, String parameter, JdbcType jdbcType)
            throws SQLException {
        Long colorId = RuntimeVariables.getInstructionOptionToIdMap().get(parameter);
        if (colorId == null) {
            ps.setLong(i, 0);
        }
        else {
    ...
    public String getNullableResult(ResultSet rs, String columnName) throws SQLException {
        ...
        if (colorId == 0) {
            ...
            return RuntimeVariables.getIdToInstructionOptionMap().get(colorId);
        }
    }
