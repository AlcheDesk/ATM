...
public class CustomServiceUnavailableException extends ServiceUnavailableException {
    ...
    public CustomServiceUnavailableException(Exception ex, String message, String trace, String code,
            UUID uuid) {
        this.message = message;
        this.trace = trace;
        this.type = ex == null ? ServiceUnavailableException.class.getName() : ex.getClass().getName();
        this.uuid = uuid;
        this.code = code == null ? Constants.APP_NAME + HttpServletResponse.SC_SERVICE_UNAVAILABLE
                : Constants.APP_NAME + HttpServletResponse.SC_SERVICE_UNAVAILABLE + code;
